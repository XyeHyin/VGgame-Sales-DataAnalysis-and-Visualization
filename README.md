...
该仓库已实现围绕 `vgsales.csv` 数据集的完整工作流：从采集、ML 加持的独立清洗、入库、再到数据库驱动的分析、可视化与机器学习建模。分析阶段默认直接从 PostgreSQL 表 `vgchartz_clean` 读取整洁数据，仅在数据库不可用时回退至 CSV。输出更新为 19 张静态图表（含季度日历、六边形密度、平台生命周期、区域扩散、瀑布图等高级图），1 份交互式 HTML 仪表盘以及多份 ML/分析 JSON，且所有描述均为中文。
