...
from src.database import PostgresConfig, PostgresReader
from src.ml import MLArtifacts, SalesMLAnalyzer
from src.plots import FigureGenerator
...
class VGSalesPipeline:
    ...
    def _write_metrics_json(self, metrics: Dict[str, object]) -> None:
        self.artifacts.metrics_json.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _write_summary(self, metrics: Dict[str, object]) -> None:
        self.artifacts.summary_md.parent.mkdir(parents=True, exist_ok=True)
        lines = [
            "# 全球电子游戏销量分析简报",
            "",
